...
trap 'rm -rf "${T}"' EXIT
mkdir -p "${T}/etc/router-ip-push-hardening" "${T}/etc/router-ip-push/routers.d" "${T}/var/lib/router-ip-push/ips" "${T}/var/lib/router-ip-push/state"
...
printf '%s\n' '198.51.100.10' >"${T}/var/lib/router-ip-push/ips/ROUTER_A.ipv4"
printf '%s\n' '198.51.100.20' >"${T}/var/lib/router-ip-push/ips/ROUTER_B.ipv4"
printf '%s\n' '198.51.100.30' >"${T}/var/lib/router-ip-push/ips/UNREGISTERED.ipv4"
cat >"${T}/etc/router-ip-push/routers.d/ROUTER_B.json" <<'EOF_B'
{"version":1,"router_id":"ROUTER_B","public_key":"ssh-ed25519 AAAA"}
EOF_B
cat >"${T}/etc/router-ip-push/routers.d/ROUTER_C.json" <<'EOF_C'
{"version":1,"router_id":"ROUTER_C","public_key":"ssh-ed25519 BBBB"}
EOF_C
...
[[ "${IDS}" == 'ROUTER_A ROUTER_B' ]] || fail "effective router ids mismatch: ${IDS}"
grep -Fq 'registered router ROUTER_C has no current IPv4 yet' "${T}/rad1.err" || fail 'registered router without first push was not reported as waiting'
[[ "${IDS}" != *UNREGISTERED* ]] || fail 'unregistered .ipv4 file became trusted'
[[ "${IDS}" != *ROUTER_C* ]] || fail 'registered router without current IP became active'
...
IDS="$(load_ids)"
[[ "${IDS}" == 'ROUTER_A' ]] || fail "disabled auto-discovery still added routers: ${IDS}"
...
{"version":1,"router_id":"OTHER_ROUTER","public_key":"ssh-ed25519 CCCC"}
EOF_BAD
if load_ids >"${T}/rad3.out" 2>"${T}/rad3.err"; then fail 'invalid registration was accepted'; fi
grep -Fq 'invalid Router IP Push registration' "${T}/rad3.err" || fail 'invalid registration refusal message missing'
