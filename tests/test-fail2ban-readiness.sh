...
make_case() {
    local t="$1"
    mkdir -p "${t}/etc/router-ip-push-hardening" "${t}/etc/fail2ban/jail.d" "${t}/etc/fail2ban/filter.d" "${t}/etc/nginx/stream-enabled" "${t}/var/lib/router-ip-push/ips" "${t}/var/log/nginx" "${t}/bin"
...
unit="\${1:-}"
case "\${cmd}:\${unit}" in
    is-enabled:riph-router-ip.path|is-active:riph-router-ip.path|is-enabled:riph-reconcile.timer|is-active:riph-reconcile.timer) exit 0;;
...
if run_readiness "${CASE_ALLOW}" "${CASE_ALLOW}/out.txt"; then fail 'Fail2ban readiness accepted allowlist missing current Router IP'; fi
