...
unit="${1:-}"
case "${cmd}:${unit}" in
    is-active:router-ip-push-nginx-hotfix.path|is-enabled:router-ip-push-nginx-hotfix.path) exit 0;;
    *) exit 1;;
esac
EOF_SYSTEMCTL
...
OUT="${T}/out.txt"
if bash "${APPLY}" --root "${T}" --reason 'must refuse dual owner' >"${OUT}" 2>&1; then fail 'riph-apply unexpectedly ran while temporary hotfix owned the allowlist'; fi
grep -Fq 'temporary Router IP Push Nginx hotfix still owns the allowlist' "${OUT}" || fail 'ownership refusal message missing'
[[ ! -e "${T}/var/lib/router-ip-push-hardening" ]] || fail 'ownership guard ran too late; RIPH state directory was already created'
