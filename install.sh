...
# but they still belong to the install transaction. Snapshot them before any
# project file is installed so a later Fail2ban/service failure can return the
# complete pre-install network state.
